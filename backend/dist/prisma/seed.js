...
async function main() {
    ...
    const teacher1 = await prisma.user.create({
        data: {
            ...
            passwordHash: await bcryptjs_1.default.hash('teacher123', 10),
    ...
    const students = await Promise.all([
        prisma.user.create({ data: { name: 'Emma Johnson', email: 'emma.johnson@student.com', passwordHash: studentPassword, role: client_1.UserRole.CHILD, teacherId: teacher1.id } }),
        prisma.user.create({ data: { name: 'Liam Smith', email: 'liam.smith@student.com', passwordHash: studentPassword, role: client_1.UserRole.CHILD, teacherId: teacher1.id } }),
        prisma.user.create({ data: { name: 'Olivia Brown', email: 'olivia.brown@student.com', passwordHash: studentPassword, role: client_1.UserRole.CHILD, teacherId: teacher1.id } }),
        prisma.user.create({ data: { name: 'Noah Davis', email: 'noah.davis@student.com', passwordHash: studentPassword, role: client_1.UserRole.CHILD, teacherId: teacher1.id } }),
        prisma.user.create({ data: { name: 'Ava Wilson', email: 'ava.wilson@student.com', passwordHash: studentPassword, role: client_1.UserRole.CHILD, teacherId: teacher1.id } }),
        prisma.user.create({ data: { name: 'Sophia Martinez', email: 'sophia.martinez@student.com', passwordHash: studentPassword, role: client_1.UserRole.CHILD, teacherId: teacher2.id } }),
        prisma.user.create({ data: { name: 'Jackson Lee', email: 'jackson.lee@student.com', passwordHash: studentPassword, role: client_1.UserRole.CHILD, teacherId: teacher2.id } }),
        prisma.user.create({ data: { name: 'Isabella Garcia', email: 'isabella.garcia@student.com', passwordHash: studentPassword, role: client_1.UserRole.CHILD, teacherId: teacher2.id } }),
    ]);
    ...
    for (let i = 1; i <= 6; i++) {
        skills.push(await prisma.microSkill.create({
            data: {
                ...
                gameTemplate: client_1.GameTemplate.TAP_SELECT,
    ...
    for (let i = 1; i <= 8; i++) {
        skills.push(await prisma.microSkill.create({
            data: {
                ...
                gameTemplate: client_1.GameTemplate.DRAG_DROP,
    ...
    for (let i = 1; i <= 3; i++) {
        skills.push(await prisma.microSkill.create({
            data: {
                ...
                gameTemplate: client_1.GameTemplate.AUDIO_TO_LETTER,
    ...
    for (let i = 1; i <= 4; i++) {
        skills.push(await prisma.microSkill.create({
            data: {
                ...
                gameTemplate: client_1.GameTemplate.SEQUENCING,
        ...
        for (let i = 1; i <= 4; i++) {
            ...
            skills.push(await prisma.microSkill.create({
                data: {
                    ...
                    gameTemplate: client_1.GameTemplate.PICTURE_TO_WORD,
    ...
    for (let i = 1; i <= 15; i++) {
        skills.push(await prisma.microSkill.create({
            data: {
                ...
                gameTemplate: client_1.GameTemplate.MEMORY_CARD,
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            skills.push(await prisma.microSkill.create({
                data: {
                    ...
                    gameTemplate: client_1.GameTemplate.FIND_THE_WORD,
                    prerequisiteSkills: JSON.parse(`["${prevCode}"]`),
                    nextSkills: JSON.parse(`["${nextCode}"]`),
