    ...
    try {
        ...
        for (const skill of skills) {
            ...
            for (let difficulty = 1; difficulty <= 3; difficulty++) {
                for (let i = 1; i <= 10; i++) {
                    const questionData = generateQuestionByTemplate(skill.code, skill.name, skill.gameTemplate, difficulty, i);
...
function generateQuestionByTemplate(skillCode, skillName, gameTemplate, difficulty, questionNum) {
...
function generateTapSelectQuestion(skillCode, difficulty, questionNum, isPhonics, isCVC, isSightWord) {
    if (isPhonics) {
...
function generateDragDropQuestion(skillCode, difficulty, questionNum, isPhonics, isCVC) {
    if (isPhonics) {
...
function generateAudioToLetterQuestion(skillCode, difficulty, questionNum, isPhonics) {
...
function generateMemoryCardQuestion(skillCode, difficulty, questionNum, isPhonics, isCVC) {
    if (isPhonics) {
        const pairs = [
            ['A', 'a'], ['B', 'b'], ['C', 'c'], ['D', 'd'], ['E', 'e']
        ];
...
function generateSortingQuestion(skillCode, difficulty, questionNum, isPhonics, isCVC) {
    if (isPhonics) {
        const vowels = ['a', 'e', 'i', 'o', 'u'];
...
function generatePictureToWordQuestion(skillCode, difficulty, questionNum, isCVC, isSightWord) {
    if (isCVC) {
        const words = [
            { word: 'cat', emoji: '🐱' },
            { word: 'dog', emoji: '🐕' },
...
function generatePuzzleJoinQuestion(skillCode, difficulty, questionNum, isCVC) {
    const words = [
...
function generateFindTheWordQuestion(skillCode, difficulty, questionNum, isSightWord, isCVC) {
    const sentences = [
...
function generateSequencingQuestion(skillCode, difficulty, questionNum, isReading, isCVC) {
    const sequences = [
...
function generateOddOneOutQuestion(skillCode, difficulty, questionNum, isPhonics, isCVC) {
    if (isPhonics) {
        const sets = [
