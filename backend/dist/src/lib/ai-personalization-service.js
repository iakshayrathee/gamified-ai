...
const openaiService = openai_service_1.default;
// ============================================
// AI PERSONALIZATION SERVICE
// ============================================
class AIPersonalizationService {
     ...
     */
    static async analyzeAnswerPatterns(childId, skillId, attempts) {
        ...
        const confusionErrors = attempts.filter(a => a.errorType !== client_1.ErrorType.NONE && a.errorType !== client_1.ErrorType.OTHER);
...
Format your response as JSON:
{
  ...
  "confusionAreas": ["area1"]
     ...
     */
    static async generatePersonalizedInsights(childId, skillId, attempts) {
        if (attempts.length === 0) {
            return {
        ...
        if (accuracy < 60)
            weakAreas.push('Accuracy needs improvement');
        if (avgTime > 6)
            weakAreas.push('Response time is slow');
        if (confusionPatterns.length > 0)
            weakAreas.push('Letter confusion detected');
     ...
     */
    static async recommendNextSkill(childId, currentSkillId, masteryData) {
        // Get current skill
        ...
        const nextSkills = currentSkill.nextSkills;
     ...
     */
    static async identifyWeakAreas(childId, attempts) {
        ...
        const calculateScore = (atts) => {
            if (atts.length === 0)
                return 100;
            return (atts.filter(a => a.isCorrect).length / atts.length) * 100;
        };
        const identifyIssues = (atts, type) => {
            const issues = [];
            const accuracy = calculateScore(atts);
            if (accuracy < 60)
                issues.push(`Low ${type} accuracy`);
            const confusionErrors = atts.filter(a => a.errorType !== client_1.ErrorType.NONE && a.errorType !== client_1.ErrorType.OTHER);
        ...
        if (phonicsScore < 70)
            recommendations.push('Focus on phonics practice with letter-sound associations');
        if (cvcScore < 70)
            recommendations.push('Practice CVC word blending and segmentation');
        if (sightWordScore < 70)
            recommendations.push('Increase sight word recognition practice');
        return {
     ...
     */
    static detectLearningTrend(attempts) {
     ...
     */
    static identifyConfusionPatterns(attempts) {
        const patterns = [];
        const confusionCounts = {};
        attempts.forEach(a => {
            if (a.errorType !== client_1.ErrorType.NONE && a.errorType !== client_1.ErrorType.OTHER) {
     ...
     */
    static generateFocusAreas(masteryLevel, weakAreas, confusionPatterns) {
        const focus = [];
     ...
     */
    static ruleBasedPatternAnalysis(attempts, accuracy, avgTime) {
        const patterns = [];
        const recommendations = [];
        ...
        // Analyze patterns
        if (accuracy >= 80)
            patterns.push('Demonstrates strong understanding');
        else if (accuracy >= 60)
            patterns.push('Shows developing skills');
        else
            patterns.push('Needs additional support');
        if (avgTime < 3)
            patterns.push('Quick response time');
        else if (avgTime > 6)
            patterns.push('Takes time to process questions');
...
exports.default = AIPersonalizationService;
