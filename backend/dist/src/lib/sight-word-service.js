...
exports.SightWordService = void 0;
...
class SightWordService {
     ...
     */
    static getAdaptiveDifficulty(tier, stage) {
        const difficultyMap = {
     ...
     */
    static analyzeErrorPatterns(attempts) {
     ...
     */
    static calculateRiskIndicator(errorPatterns, tier) {
        const riskFactors = Object.values(errorPatterns).filter(v => v === true).length;
        if (tier === 1 && riskFactors === 0)
            return 'Low';
        if (tier === 1 && riskFactors <= 1)
            return 'Low';
        if (tier === 2 && riskFactors <= 1)
            return 'Medium';
        if (tier === 2 && riskFactors >= 2)
            return 'Medium';
        if (tier === 3 || riskFactors >= 3)
            return 'High';
        return 'Medium';
    }
    /**
     ...
     */
    static calculateOverallTier(wordMasteryData) {
        if (wordMasteryData.length === 0) {
            return this.calculateTier(0);
        }
        const totalAccuracy = wordMasteryData.reduce((sum, wm) => sum + wm.accuracyPercentage, 0);
     ...
     */
    static async getProgressSummary(childId, skillId) {
     ...
     */
    static async canProgressToNextStage(childId, currentSkillCode) {
...
exports.SightWordService = SightWordService;
exports.default = SightWordService;
