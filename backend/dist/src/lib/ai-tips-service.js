...
class AITipsService {
     ...
     */
    static async generateBehavioralTip(childId, recentAttempts, forceAI = false) {
     ...
     */
    static analyzeBehavioralPattern(attempts) {
        ...
        if (attempts.length >= 6) {
            ...
            if (secondAcc - firstAcc > 15)
                accuracyTrend = 'improving';
            else if (secondAcc - firstAcc < -15)
        ...
        const errorPattern = mostCommonError !== client_1.ErrorType.NONE && mostCommonError !== client_1.ErrorType.OTHER
            ? mostCommonError
            : null;
        // Hint dependency
        const hintUsageRate = (attempts.filter(a => a.hintUsed).length / attempts.length) * 100;
        let hintDependency = 'low';
        if (hintUsageRate > 50)
            hintDependency = 'high';
        else if (hintUsageRate > 25)
     ...
     */
    static getRuleBasedTip(pattern, attempts) {
        const accuracy = (attempts.filter(a => a.isCorrect).length / attempts.length) * 100;
     ...
     */
    static async getAITip(childId, pattern, attempts) {
        // Check cache first
...
Give ONE short, encouraging tip (max 15 words) for a child. Be positive and actionable.`;
        try {
            ...
            const response = await openaiService.generateTextResponse(prompt, 'You are a supportive teacher. Be brief and encouraging.', 'gpt-4o-mini' // Use cheaper model
            );
     ...
     */
    static async generateSessionAssessment(childId, sessionAttempts) {
...
Write 1 sentence assessment for teacher (max 20 words).`;
        try {
            const assessment = await openaiService.generateTextResponse(prompt, 'You are an educator writing brief assessments.', 'gpt-4o-mini');
...
exports.AITipsService = AITipsService;
exports.default = AITipsService;
