...
exports.QuizReviewService = void 0;
...
class QuizReviewService {
     ...
     */
    async generateQuizReviewWithRecommendation(sessionId, childId, autoSave = true) {
        try {
            ...
            const masteredSkillIds = new Set(skillProgress.filter(p => p.masteryStatus === 'MASTERED').map(p => p.microSkillId));
...
Keep all text concise but meaningful. Focus on what they did well and one key improvement area.`;
            try {
                ...
                if (autoSave) {
                    await this.saveQuizReview(sessionId, childId, currentSkillId, review, accuracy, totalAttempts, correctAttempts, avgResponseTime, recommendation);
                }
                ...
                if (autoSave) {
                    await this.saveQuizReview(sessionId, childId, currentSkillId, fallbackReview, accuracy, totalAttempts, correctAttempts, avgResponseTime, fallbackRecommendation);
                }
                return {
     ...
     */
    async saveQuizReview(sessionId, childId, skillId, review, accuracy, totalAttempts, correctAttempts, avgResponseTime, recommendation) {
        try {
     ...
     */
    async generateQuizReview(sessionId, childId) {
     ...
     */
    async recommendNextSkill(sessionId, childId) {
        try {
            // Get session info
            const session = await prisma.session.findUnique({
                where: { id: sessionId },
...
5. If they struggled (<60%), reinforce fundamentals`;
     ...
     */
    getFallbackReview(accuracy, correct, total, confusionPatterns) {
...
exports.QuizReviewService = QuizReviewService;
exports.default = new QuizReviewService();
