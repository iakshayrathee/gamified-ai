...
exports.ResultsAnalyticsService = void 0;
 ...
 */
function areVisuallySimilar(word1, word2) {
...
class ResultsAnalyticsService {
     ...
     */
    async calculateWordMastery(childId, skillId) {
     ...
     */
    async calculateListReadiness(childId, skillId) {
        ...
        const readinessScore = Math.round((accuracyScore * 0.5) + (speedScore * 0.3) + (consistencyScore * 0.2));
     ...
     */
    detectVisualConfusion(attempts) {
        ...
        if (totalConfusions >= 10)
            severity = 'high';
        else if (totalConfusions >= 5)
            severity = 'medium';
     ...
     */
    detectRandomGuessing(attempts) {
        const rapidIncorrect = attempts.filter(a => !a.isCorrect && a.responseTimeSeconds < 2);
     ...
     */
    detectInconsistentPerformance(attempts) {
     ...
     */
    detectAvoidanceBehavior(attempts) {
        const timeouts = attempts.filter(a => a.responseTimeSeconds >= 30);
        const rapidIncorrectOnHard = attempts.filter(a => !a.isCorrect &&
     ...
     */
    async detectErrorPatterns(attempts) {
        return {
     ...
     */
    async analyzeClusterPerformance(childId, skillId) {
        const wordMasteryList = await this.calculateWordMastery(childId, skillId);
     ...
     */
    generateGameRecommendations(errorPatterns) {
     ...
     */
    calculateRepetitionSchedule(wordMasteryList) {
        const schedule = [];
        const now = new Date();
     ...
     */
    async generateDetailedReport(childId, sessionId) {
        // Get session data
...
exports.ResultsAnalyticsService = ResultsAnalyticsService;
exports.default = new ResultsAnalyticsService();
