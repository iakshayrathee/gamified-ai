...
exports.starsToCoins = starsToCoins;
...
class AdaptiveDifficultyEngine {
     ...
     */
    static adjustDifficulty(recentAttempts, currentLevel) {
     ...
     */
    static checkMastery(last10Attempts, criteria = {
        ...
        const confusionErrors = last10Attempts.filter(a => a.errorType === client_1.ErrorType.B_D_CONFUSION ||
            a.errorType === client_1.ErrorType.P_Q_CONFUSION ||
            a.errorType === client_1.ErrorType.M_N_CONFUSION ||
            a.errorType === client_1.ErrorType.U_N_CONFUSION).length;
        ...
        if (mastered) {
            ...
            if (!meetsAccuracyThreshold)
                issues.push(`accuracy ${accuracy.toFixed(0)}% < ${criteria.accuracyThreshold}%`);
            if (!meetsTimeThreshold)
                issues.push(`avg time ${avgResponseTime.toFixed(1)}s > ${criteria.timeThreshold}s`);
            if (!meetsConfusionThreshold)
                issues.push(`confusion rate ${confusionErrorRate.toFixed(0)}% ≥ ${criteria.confusionErrorThreshold}%`);
     ...
     */
    static recommendNextSkill(currentSkill, masteryStatus, allSkills) {
     ...
     */
    static analyzeWeakAreas(attemptsByDomain, domains) {
        const weakAreas = [];
     ...
     */
    static detectConfusionPattern(attempts) {
        const confusionErrors = attempts.filter(a => a.errorType === client_1.ErrorType.B_D_CONFUSION ||
            a.errorType === client_1.ErrorType.P_Q_CONFUSION ||
            a.errorType === client_1.ErrorType.M_N_CONFUSION ||
            a.errorType === client_1.ErrorType.U_N_CONFUSION);
     ...
     */
    static suggestIntervention(domain, accuracy) {
     ...
     */
    static hasConsecutiveErrors(attempts, count) {
     ...
     */
    static analyzeAttemptInRealTime(recentAttempts, currentDifficulty) {
        const insights = [];
     ...
     */
    static getAdaptiveDifficultyForNextQuestion(last5Attempts, currentDifficulty) {
     ...
     */
    static generateDomainRecommendations(domain, accuracy) {
 ...
 */
function calculateStars(isCorrect, responseTime, hintUsed) {
    if (!isCorrect)
        return 0;
    let stars = 1.0;
 ...
 */
function classifyError(correctAnswer, userResponse) {
    ...
    if (vowels.includes(correct) && vowels.includes(response) && correct !== response) {
        return client_1.ErrorType.VOWEL_ERROR;
    }
