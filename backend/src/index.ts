 ...
 */
app.post('/api/attempts/batch', async (req: Request, res: Response) => {
    try {
        ...
        for (const attemptData of attempts) {
            try {
                ...
                await prisma.$transaction(async (tx) => {
                    ...
                    const updatedPatterns: string[] = Array.isArray(existingPatterns)
        ...
        if (savedAttemptIds.length > 0) {
            setImmediate(async () => {
                try {
                    ...
                    const uniqueCombos = new Map<string, { childId: string; microSkillId: string }>();
                    for (const attemptData of attempts) {
 ...
 */
async function calculateTierForSkill(childId: string, microSkillId: string) {
    try {
        // Get skill info
        const skill = await prisma.microSkill.findUnique({
