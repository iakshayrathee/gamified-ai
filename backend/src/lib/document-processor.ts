...
import { uploadAsset } from './s3-service';
...
interface ExtractedQuestionData {
    ...
    gameTemplate?: string;      // AI-suggested game template
    difficultyLevel?: number;   // AI-suggested difficulty (1-3)
}
...
Return an empty array ONLY if there is absolutely no educational content.`;
    ...
    private async uploadImages(
        ...
        for (let i = 0; i < images.length; i++) {
            try {
                ...
                const processedImage = await sharp(images[i])
        ...
        for (const q of questions) {
            await prisma.extractedQuestion.create({
                data: {
                    ...
                    gameTemplate: (q.gameTemplate as any) || 'TAP_SELECT',
     ...
     */
    async getAllDocuments(limit: number = 50): Promise<any[]> {
    ...
    async approveQuestion(
        ...
        try {
            ...
            const options = Array.isArray(extractedQuestion.options)
                ? extractedQuestion.options as string[]
                : JSON.parse(extractedQuestion.options as any) as string[];
     ...
     */
    async rejectQuestion(questionId: string): Promise<any> {
