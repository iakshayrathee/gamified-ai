...
export class ErrorPatternAnalyzer {
    ...
    static analyzeAttempt(
        ...
        userResponse: string,
        context?: { sentence?: string; stage?: string }
    ): ErrorType {
        ...
        if (context?.stage === 'Reading' && context?.sentence) {
            if (this.isContextualError(correctAnswer, userResponse, context.sentence)) {
                return ErrorType.CONTEXTUAL_ERROR;
            }
        }
     ...
     */
    private static hasVisualConfusion(correct: string, response: string): boolean {
     ...
     */
    private static detectConfusionType(correct: string, response: string): ErrorType {
     ...
     */
    private static isContextualError(correct: string, response: string, sentence: string): boolean {
     ...
     */
    private static isSpellingReversal(correct: string, response: string): boolean {
        if (correct.length !== response.length) return false;
     ...
     */
    private static isSpellingOmission(correct: string, response: string): boolean {
     ...
     */
    private static hasLetterSequencingError(correct: string, response: string): boolean {
...
export class AIAnalyticsService {
     ...
     */
    static async identifyWeakClusters(childId: string, skillId: string): Promise<any[]> {
        ...
        for (const wm of wordMastery) {
            ...
            if (vowels && vowels.length > 0) {
                ...
                if (!clusters.has(vowelKey)) clusters.set(vowelKey, []);
     ...
     */
    private static getRecommendation(pattern: string): string {
        if (pattern.includes('letter words')) return 'Practice word length recognition';
        if (pattern.includes('starting with')) return 'Focus on initial letter sounds';
        if (pattern.includes('vowels')) return 'Strengthen vowel recognition';
     ...
     */
    static async calculateReadinessScore(childId: string): Promise<number> {
        const stages = [
     ...
     */
    static async generateWordLists(childId: string, skillId: string): Promise<{
     ...
     */
    static async generateInsights(childId: string, skillId: string): Promise<string[]> {
        ...
        const visualErrorTypes: ErrorType[] = [
            ErrorType.B_D_CONFUSION,
            ErrorType.P_Q_CONFUSION,
            ErrorType.M_N_CONFUSION,
            ErrorType.U_N_CONFUSION
        ];
        const visualErrors = recentAttempts.filter(a =>
            visualErrorTypes.includes(a.errorType)
        ).length;
...
export default { ErrorPatternAnalyzer, AIAnalyticsService };
